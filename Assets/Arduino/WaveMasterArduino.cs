...
public class WaveMasterArduino : MonoBehaviour
{
		...
		void Start ()
		{
				...
				max_width = (int)FixCamera._fixWidth / interval_width;
				for (int i = 0; i < max_width; i++) {
						wave_point_x.AddLast (FixCoordinates.Instance.GetPointX (i * interval_width));
				}

				max_height = (int)FixCamera._fixHeight;
		...
		void OnPostRender ()
		{
				...
				m_draw2D.Begin ();
				{
						...
						Line [0] = new Vector3 (FixCoordinates.Instance.GetPointX (max_width * interval_width), FixCoordinates.Instance.GetPointY (max_height / 2), 0.0f);
						Line [1] = new Vector3 (FixCoordinates.Instance.GetPointX (0), FixCoordinates.Instance.GetPointY (max_height / 2), 0.0f);
